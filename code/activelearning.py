...
class ActiveLearning(object):
    ...
    def __init__(self, conf, pose_net, aux_net): #(self, conf, hg_network, learnloss_network):
        ...
        if conf.active_learning['algorithm'] in ['learning_loss', 'uncertainty_BN', 'uncertainty_KendallGal']:
            self.aux_net.eval()
    ...
    def base(self, train, dataset_size):
        ...
        if self.conf.dataset['load'] == 'mpii':
            ...
            selection_lspet = np.random.choice(np.arange(dataset_size['lsp']['train'], dataset_size['lspet']['train']),
    ...
    def coreset_sampling(self, train, dataset_size):
        ...
        coreset_dataloader = torch.utils.data.DataLoader(dataset_, batch_size=128, shuffle=False, num_workers=1)
    ...
    def learning_loss_sampling(self, train, dataset_size):
        ...
        for key in train.keys():
            if key == 'img' and (not self.conf.dataset['load_all_imgs_in_memory']):
                unlabelled_dataset[key] = train[key]
                continue

            unlabelled_dataset[key] = train[key][unlabelled_idx]

        dataset_ = ActiveLearningDataset(unlabelled_dataset, indices=np.arange(unlabelled_dataset['name'].shape[0]), conf=self.conf)
    ...
    def expected_gradient_length_sampling(self, train, dataset_size):
        ...
        with torch.no_grad():
            ...
            #print(p_i_given_j.shape)
            #print(p_i_given_j.max(dim=1)[0])
            #print(p_i_given_j.min(dim=1)[0])
            #print(idx.shape)
            #print()
    ...
    def multipeak_entropy(self, train, dataset_size):
        ...
        for i in tqdm(range(pose_heatmaps.shape[0])):
            ...
            for hm in range(pose_heatmaps.shape[1]):
                loc = peak_local_max(pose_heatmaps[i, hm], min_distance=5, threshold_abs=5)
...
class ActiveLearningDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dict, indices, conf):
        ...
        :param dataset_dict:
        '''
        if conf.dataset['load_all_imgs_in_memory']:
            self.images = dataset_dict['img'][indices]
        else:
            self.names = dataset_dict['name'][indices]

        self.load_all_imgs = conf.dataset['load_all_imgs_in_memory']
