...
def visualize_image(image_info, save_dir, jnt_to_ind, bbox=False, adjusttext=False):
    '''
    ...
    colour = {'rankl': (0, 0, 1), 'rknee': (0, 0, 1), 'rhip': (0, 0, 1),
              'lankl': (1, 0, 0), 'lknee': (1, 0, 0), 'lhip': (1, 0, 0), 'pelvis': (1, 0, 0),
              'rwri': (1, 1, 0), 'relb': (1, 1, 0), 'rsho': (1, 1, 0),
              'lwri': (0, 1, 0), 'lelb': (0, 1, 0), 'lsho': (0, 1, 0),
              'head': (0, 1, 1), 'neck': (0, 1, 1), 'thorax': (0, 1, 1)}
    ...
    for dataset_name_ in image_info.keys():
        ...
        for i in range(len(image_info[dataset_name_]['img'])):
            ...
            joint_names = list(jnt_to_ind.keys())
...
def fast_argmax(_heatmaps):
    ...
    indices = torch.cat(((indices // 64).view(batch_size, num_jnts, 1), (indices % 64).view(batch_size, num_jnts, 1)),
                        dim=2)
