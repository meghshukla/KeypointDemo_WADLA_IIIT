...
NUM_IMAGES_TO_VISUALIZE_JUPYTER = 10
...
class Train(object):
    def __init__(self, pose_model, aux_net, hyperparameters, dataset_obj, conf, tb_writer):
        ...
        :param conf: (Object of IncrementalConfiguration) Contains the configurations for the model
    ...
    def train_model(self):
        ...
        for e in range(self.start_epoch, self.epoch):
            ...
            for (images, heatmaps, _, _, _, gt_per_image, split, _, _, _, joint_exist) in tqdm(self.torch_dataloader):
                ...
                if self.conf.model['aux_net']['train'] and self.conf.model['aux_net']['method'] == 'uncertainty_KendallGal':
    ...
    def learning_loss(self, pose_encodings, true_loss, gt_per_img, epoch):
        ...
        Based on the paper: "Learning Loss For Active Learning, CVPR 2019"
        :param hg_encodings: (Dict of tensors) Intermediate (Hourglass) and penultimate layer output of the M10 network
...
class Metric(object):
    ...
    def inference(self):
        ...
        global NUM_IMAGES_TO_VISUALIZE_JUPYTER
            ...
            for (images, _, gt, name, dataset, num_gt, split, _, scale_params, normalizer, joint_exist) in tqdm(
                ...
                if self.conf.viz:
                    ...
                    for i in range(images.shape[0]):
                        ...
                        if NUM_IMAGES_TO_VISUALIZE_JUPYTER > 0:
                            plt.subplot(4, 4, 1)
                            plt.imshow(images[i].numpy())
                            plt.axis('off')
                            plt.show()

                            # Overlay predicted heatmaps on image
                            for j in range(outputs.cpu().numpy()[i].shape[0]):
                                plt.subplot(4, 4, j + 1)
                                plt.imshow(images[i].numpy())
                                plt.subplot(4, 4, j + 1)
                                plt.imshow(cv2.resize(outputs.cpu()[i][j].numpy(), dsize=(256, 256),
                                                      interpolation=cv2.INTER_CUBIC), alpha=.5)
                                plt.title('{}'.format(self.dataset_obj.ind_to_jnt[j]), fontdict={'fontsize': 6})
                                plt.axis('off')

                            plt.show()
                            plt.close()
                            NUM_IMAGES_TO_VISUALIZE_JUPYTER -= 1
    ...
    def visualize_predictions(self, image=None, name=None, dataset=None, gt=None, pred=None):
        ...
        visualize_image(dataset_viz, save_dir=self.conf.model['save_path'], jnt_to_ind=self.dataset_obj.jnt_to_ind, bbox=False)
...
def debug_viz(data_obj):
    ...
    for i in range(30, 40):
        ...
        for j in range(hm.shape[0]):
            ...
            plt.title('{}'.format(data_obj.ind_to_jnt[j]))
            plt.show()
            plt.close()
