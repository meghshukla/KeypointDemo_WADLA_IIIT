...
def load_mpii(mpii_conf, del_extra_jnts, load_all_imgs_in_memory):
    """
    ...
    for img_idx in tqdm(range(matlab_mpii.__dict__['annolist'][0].shape[0])):
        ...
        if load_all_imgs_in_memory: img_dict['mpii']['img'].append(image)
...
def load_lsp(lsp_conf, model_conf, load_all_imgs_in_memory):#shuffle=False, train_ratio=0.7, conf=None):
    """

    :param lsp_conf:
    :param model_conf:
    :param load_all_imgs_in_memory:
    :return:
    """
    root = Path(os.getcwd()).parent
    ...
    for index in tqdm(range(annotation_lsp.shape[2])):
        ...
        if load_all_imgs_in_memory: img_dict['lsp']['img'].append(image)

    return img_dict


def load_lspet(lspet_conf, model_conf, load_all_imgs_in_memory):
    """
    ...
    for index in tqdm(range(annotation_lspet.shape[2])):
        ...
        if load_all_imgs_in_memory: img_dict['lspet']['img'].append(image)
...
def load_hp_dataset(dataset_conf, model_conf):
    ...
    if dataset_conf['load'] == 'mpii' or dataset_conf['load'] == 'merged':
        ...
        dataset_dict.update(load_mpii(mpii_conf=dataset_conf['mpii_params'],
                                      ...
                                      load_all_imgs_in_memory=dataset_conf['load_all_imgs_in_memory']))
...
class HumanPoseDataLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_dict, activelearning, conf):
        ...
        activelearning_samplers = {
            ...
            'entropy': activelearning.multipeak_entropy,
        }
    ...
    def __getitem__(self, i):
        ...
        if self.conf.dataset['load_all_imgs_in_memory']:
    ...
    def create_mpii(self):#(self, train_ratio=0.7, max_persons=0, shuffle=False):
        ...
        for i in range(len_dataset):
            ...
            if self.conf.dataset['load_all_imgs_in_memory']: image = mpii['img'][i]
            ...
            if self.conf.dataset['load_all_imgs_in_memory']: dataset['img'].append(image)
        ...
        dataset['img'] = np.array(dataset['img']) # array of shape == 0 when load_all_imgs_in_memory == False
    ...
    def create_lspet(self):
        ...
        for i in range(len_dataset):
            ...
            if self.conf.dataset['load_all_imgs_in_memory']: image = lspet['img'][i]
    ...
    def create_lsp(self):
        ...
        for i in range(len_dataset):
            ...
            if self.conf.dataset['load_all_imgs_in_memory']: image = lsp['img'][i]
    ...
    def create_train_validate(self, dataset=None):
        ...
        for key in dataset.keys():
            if key == 'img' and (not self.conf.dataset['load_all_imgs_in_memory']):
    ...
    def mpii_single_person_extractor(self, mpii_dataset):
        ...
        for i in range(len(mpii_dataset['name'])):
            for p in range(int(mpii_dataset['num_persons'][i][0])):
                if self.conf.dataset['load_all_imgs_in_memory']: dataset['img'].append(mpii_dataset['img'][i])
